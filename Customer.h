...
public:
    ...
    Customer(const string& id, const string& name, const string& phoneNumber,
        ...
        : customerId(id), fullName(name), phone(phoneNumber),
          address(customerAddress), username(user), password(pass) {}

    ~Customer() {
        for (BankAccount* account : accounts) delete account;
        accounts.clear();
    }
    ...
    void setPhone(const string& number) { phone = number; }
    ...
    void save(ofstream& file) const {
        ...
        for (BankAccount* account : accounts) account->save(file);
